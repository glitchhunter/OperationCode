...
#include "Int_OperatorMultiply_IntInt.h"
#include "Interpreter/CodeRunner/Values/PredefinedClasses/IntInstance.h"

UInt_OperatorMultiply_IntInt::UInt_OperatorMultiply_IntInt()
{
...
UValue* UInt_OperatorMultiply_IntInt::Run_Implementation(USimpleFunctionDefinitionRTS* RTS)
{
	int32 lhs = RTS->GetIntValue("lhs");
	int32 rhs = RTS->GetIntValue("rhs");
	int32 result = lhs * rhs;

	RTS->AddDebugMessage("Multiplying lhs value " + FString::FromInt(lhs) + " with rhs value " + FString::FromInt(rhs) + ".");
