...
#include "Bool_LessOrEqual_FloatFloat.h"
#include "Interpreter/CodeRunner/Values/PredefinedClasses/BoolInstance.h"

UBool_LessOrEqual_FloatFloat::UBool_LessOrEqual_FloatFloat()
{
...
UValue* UBool_LessOrEqual_FloatFloat::Run_Implementation(USimpleFunctionDefinitionRTS* RTS)
{
	...
	if (lhs <= rhs)
	{
		RTS->AddDebugMessage("Lhs has a value that is smaller or equal to rhs. Returning true.");
		return URTS_InstanceCreator::CreateBoolValue(RTS->GetCodeRunner(), true);
	}

	RTS->AddDebugMessage("Lhs does not have a value that is smaller or equal to rhs. Returning false.");
