...
#include "Bool_LessOrEqual_IntInt.h"

UBool_LessOrEqual_IntInt::UBool_LessOrEqual_IntInt()
{
...
UValue* UBool_LessOrEqual_IntInt::Run_Implementation(USimpleFunctionDefinitionRTS* RTS)
{
	...
	if (result) RTS->AddDebugMessage("Lhs is smaller or equal then rhs, returning true.");
	else RTS->AddDebugMessage("Lhs is not smaller or equal then rhs, returning false.");
