...
#include "Bool_GreaterOrEqual_IntInt.h"
#include "Interpreter/CodeRunner/Values/PredefinedClasses/BoolInstance.h"

UBool_GreaterOrEqual_IntInt::UBool_GreaterOrEqual_IntInt()
{
...
UValue* UBool_GreaterOrEqual_IntInt::Run_Implementation(USimpleFunctionDefinitionRTS* RTS)
{
	...
	if (result) RTS->AddDebugMessage("Lhs is greater or equal then rhs, returning true.");
	else RTS->AddDebugMessage("Lhs is not greater or equal then rhs, returning false.");
